...
# SARSA 는 epsilon-greedy poliy 에 의히서 결정되는 action을 next_state의 next_action으로 사용 
# Q-learning은 greedy-policy 에 의해서 next state에서 할 수 있는 action 중 가장 가치 가 큰 것을 선택.
def sarsa(q_value, expected=False, step_size=ALPHA):
    ...
    while state != GOAL:
        ...
        if not expected:
            ...
            # q_next 에서 value 가 max 가 되는 index 알려줌.
            best_actions = np.argwhere(q_next == np.max(q_next))
            for action_ in ACTIONS:
                # target : \Sigma \pi(a|S_{t+1})Q(S_{t+1},a)
...
def figure_6_4():
    ...
    plt.savefig('sutton_rl/chapter06/figure_6_4.png')
    plt.close()
...
def figure_6_6():
    ...
    plt.savefig('sutton_rl/chapter06/figure_6_6.png')
    plt.close()
