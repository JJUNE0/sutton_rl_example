...
def step(state, action):
    ...
    # action 은 +-1 이기에, 액션에 따라 방향을 정해주는 역할..
    step *= action
    state += step
...
def semi_gradient_temporal_difference(value_function, n, alpha):
    ...
    while True:
        ...
        if update_time >= 0:
            ...
            if update_time + n <= T:
                returns += pow(GAMMA,n)*value_function.value(states[update_time + n])
...
def figure_9_1(true_value):
    ...
    plt.savefig('sutton_rl/chapter09/figure_9_1.png')
    plt.close()
...
def figure_9_2(true_value):
    ...
    plt.savefig('sutton_rl/chapter09/figure_9_2.png')
    plt.close()
...
def figure_9_5(true_value):
    ...
    plt.savefig('sutton_rl/chapter09/figure_9_5.png')
    plt.close()
...
def figure_9_10(true_value):
    ...
    plt.savefig('sutton_rl/chapter09/figure_9_10.png')
    plt.close()
