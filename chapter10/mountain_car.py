...
# 다차원 연속 공간에 대한 엉성한 부호화의 한 예제
# 각 타일 영역은 1개의 특징 벡터를 가진다
# 전체 타일영역의 개수는 전체 특징 벡터의 개수와 동일하다.
# 일반화 진행 속도는 상태들의 공유하는 타일의 개수에 비례한다.
# 해싱은 큰 타일의 영역을 훨씬 더 작은 여러개의 타일 집합으로 나누고
# 작은 부분에서만 고해상도가 필요
...
class IHT:
    ...
    def full(self):
        # 해쉬 테이블의 길이가 해쉬 테이블의 정해진 사이즈보다 크면 true 반환
...
def figure_10_1():
    ...
    plt.savefig('sutton_rl/chapter09/figure_10_1.png')
    plt.close()
...
def figure_10_2():
    ...
    plt.savefig('sutton_rl/chapter09/figure_10_2.png')
    plt.close()
...
def figure_10_3():
    ...
    plt.savefig('sutton_rl/chapter09/figure_10_3.png')
    plt.close()
...
def figure_10_4():
    ...
    plt.savefig('sutton_rl/chapter09/figure_10_4.png')
    plt.close()
